...
@Service
public class WorkHandler implements SpreadsheetHandler{
    private final SpreadsheetService spreadsheetService;
    public WorkHandler(SpreadsheetService spreadsheetService){
        this.spreadsheetService = spreadsheetService;
    }
    @Override
    public List<List<Object>> read(String spreadsheetId, String range) throws IOException {
        // 레벨별 경험치 시트에서 데이터를 읽는 로직
        //System.out.printf("Reading data for 전사 프로젝트: Range: %s%n", range);
        //return spreadsheetService.fetchDataFromSpreadsheet(spreadsheetId, range);
        System.out.printf("Reading data for 전사 프로젝트: Range: %s%n", range);
    ...
    public void update(String spreadsheetId, String range, List<List<Object>> values) throws IOException {
        // 레벨별 경험치 시트 업데이트 로직
        System.out.printf("Updating 전사 프로젝트: Range: %s, Values: %s%n", range, values);
        // 실제 업데이트 로직 작성
    }
    ...
    public void notifyChange(String range, Object value) {
        ...
        // 여기서 시트 이름을 지정합니다.
        String sheetName = "참고. 전사 프로젝트"; // 사용할 실제 시트 이름으로 변경
