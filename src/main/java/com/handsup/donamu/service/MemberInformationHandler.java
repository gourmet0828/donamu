...
public class MemberInformationHandler implements SpreadsheetHandler{
    @Override
    public List<List<Object>> read(String spreadsheetId, String range) throws IOException {
        // 인사평가 시트에서 데이터를 읽는 로직
        System.out.printf("Reading data for 구성원 정보: Range: %s%n", range);
        // 실제 데이터 반환 로직 작성
    ...
    public void update(String spreadsheetId, String range, List<List<Object>> values) throws IOException {
        // 인사평가 시트 업데이트 로직
        System.out.printf("Updating 구성원 정보: Range: %s, Values: %s%n", range, values);
        // 실제 업데이트 로직 작성
    }
    ...
    public void notifyChange(String range, Object value) {
        // 인사평가 시트에서의 변경 사항 처리
        System.out.printf("Notifying 구성원 정보: Range: %s, New Value: %s%n", range, value);
    }
}
